<!-- office-crew-developer-conventions: v21 -->
...
4. **Tests are part of done.** Every ticket includes writing tests AND making
   them pass. `PYTHONPATH=. python3 -m pytest` must exit 0 before you stop.
...
- **PROVE IT BEFORE YOU FINISH: run `python3 _office_run_check.py` from the repo
...
2. Minimum acceptable instructions:
   - Static files only: `python3 -m http.server 8000`, then open `http://localhost:8000`
